...
- **通信可隐藏条件**: C/B ≤ 2·d_ff（每 GB/s 带宽可隐藏约 6 TFLOP/s 计算，V4-Pro）。
- **细粒度 EP (V4)**: Wave 调度 → 多级流水线重叠 Dispatch/Compute/Combine，理论加速 1.92×。
- 📖 [Expert Parallelism 深度笔记](topics/expert-parallelism.md) · [DeepSeek-V3](papers-zh/DeepSeek-V3.md)
...
- 📖 [Expert Parallelism](topics/expert-parallelism.md) (Section 7.2)
...
- 📖 [Expert Parallelism](topics/expert-parallelism.md) (Section 3)
...
### Q2: TP vs EP 什么时候用哪个？
**TP**: 层内矩阵切分，通信量 O(h²)，需要高速 NVLink。**EP**: MoE 专家切分，通信量 O(token · h)，适合跨节点。专家多 → EP 优先；单专家大 → TP 优先。
📖 [Expert Parallelism Q13](topics/expert-parallelism.md#q13-ep-和-tp-如何协作什么情况下选-ep-优先什么情况选-tp-优先)
